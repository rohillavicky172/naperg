...
class SingleProductList extends React.Component<Props, State> {
  render() {
    ...
    // let allUsersUnique = allUsers.filter((thing, index, self) =>
      ...
      <div className="paperOut">
        <Paper className="paperIn bgHover">
          <Grid container>
            ...
            {/* <Grid item xs={12} sm={2}>
              <div>
                {allUsersUnique.length} Users
              </div>
              <div>
                {allInvoices.length} Transactions
              </div>
            </Grid>            
                ...
                <div key={user.id}>
                  ...
                  {allInvoices.filter(invoice => invoice.user.id === user.id).length>0 && (
                    <>
                      ...
                      <Link className='link' to={`/invoicesCompany/${allInvoices.filter(invoice => invoice.user.id === user.id)[0].companie.id}/?productName=${this.props.product.name}&userName=${user.firstName}`}>
            ...
            <Grid item xs={1} sm={1} className="marginAuto">
