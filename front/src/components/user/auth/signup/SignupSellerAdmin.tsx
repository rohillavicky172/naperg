...
import { SIGNUP_ADMIN_SELLER_MUTATION } from '../GraphQL'
...
type Props = {
  client: Client
  signupAdminSeller: any
  context: Context
  history: any
  location: Location
}

class SignupSellerAdmin extends React.Component<Props, State> {
  state = {
    loading: false,
  ...
  _confirm = async () => {
    ...
    let result
    try {
      result = await this.props.signupAdminSeller({
        variables: {
    ...
    if (result.data.signupAdminSeller) {
      this.setState({ loading: false })
      this.resetFields()
...
export default compose(
  ...
  withRouter,
  withContext,
  graphql(SIGNUP_ADMIN_SELLER_MUTATION, { name: 'signupAdminSeller' })
)(SignupSellerAdmin)
