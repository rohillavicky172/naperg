...
class InvoicesDashboardQuery extends React.Component<Props, State> {
  render() {
    ...
    return (
      <>
        ...
        <CardActions>
          {this.props.variables.where.companie && this.props.variables.where.user && this.props.user && (
            <div>
              {this.props.invoicesQueryConnection.invoicesConnection.aggregate.count > 3 && (
                <Link className="link" to={'/invoicesCompany/' + this.props.companie.id}>
                  {`See all transactions for ${this.props.user.firstName} ${this.props.user.lastName} in ${this.props.companie.name}`}
                </Link>
              )}
            </div>
          )}
          {this.props.variables.where.companie && !this.props.variables.where.user && (
            <div>
              {this.props.invoicesQueryConnection.invoicesConnection.aggregate.count > 3 && (
                <Link className="link" to={'/invoicesCompany/' + this.props.companie.id}>
                  {`See all transactions for ${this.props.companie.name}`}
                </Link>
              )}
            </div>
          )}
